...
function sidebarModules(id, { permissions }) {
    ...
    return `
            ...
            `<div data-sidebar-body><a data-enhance href="${getUrl({ view: 'blocks.create' })}" data-sidebar-item-button data-button data-button-block data-button-color="primary">
...
function sidebarCollections(collections, id, { permissions }) {
    return `
        ...
        <div data-sidebar-body>
            ...
            ${permissions.collection_create ? `<a data-enhance href="${getUrl({ view: 'collection.create' })}" data-sidebar-item-button data-button data-button-block data-button-color="primary">
                <svg data-sidebar-item-icon xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24"><path fill="currentColor" d="M11 13H6q-.425 0-.712-.288T5 12t.288-.712T6 11h5V6q0-.425.288-.712T12 5t.713.288T13 6v5h5q.425 0 .713.288T19 12t-.288.713T18 13h-5v5q0 .425-.288.713T12 19t-.712-.288T11 18z"/></svg>
                Create Collection
            </a>` : ''}
...
async function sidebarPages(pages, currentPage, { permissions }) {
    return `
        ...
        <div data-sidebar-body>
            ${permissions.page_create ? `<a data-enhance href="${getUrl({ view: 'pages.create' })}" data-sidebar-item-button data-button data-button-block data-button-color="primary">
                <svg data-sidebar-item-icon xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24"><path fill="currentColor" d="M11 13H6q-.425 0-.712-.288T5 12t.288-.712T6 11h5V6q0-.425.288-.712T12 5t.713.288T13 6v5h5q.425 0 .713.288T19 12t-.288.713T18 13h-5v5q0 .425-.288.713T12 19t-.712-.288T11 18z"/></svg>
                Create Page
            </a>` : ''}
        </div>
...
async function BlockList() {
    ...
    return `
            <div data-sidebar-body data-definitions>
                ${modules.map(x => `<a href="${getUrl({ view: 'blocks.update', id: x.id })}" data-definition-module data-definition-id="${x.id}" ${x.owner === 'ai' ? 'data-definition-ai' : ''} >
                    <span>${x.name}</span>
                </a>`).join('')}
            </div>
            <div data-sidebar-body><a data-enhance href="${getUrl({ view: 'blocks.create' })}" data-sidebar-item-button data-button data-button-block data-button-color="primary">
...
export async function renderBody(body, { props, mode, url, view, params, ...query }) {
    ...
    if (view === 'iframe' || !view) {
        ...
        content = `
            <div data-name="iframe">
                ...
                <div data-iframe-header>
                    <div data-iframe-actions>
                        ...
                        <div data-iframe-actions-end>
                            ...
                            <a data-enhance data-button data-button-color="default" href="${getUrl({ view: 'pages.update', id: currentPage.id })}" data-button-icon>
                ...
                <iframe class="iframe" src="${url.replace('mode=edit', 'mode=view').replace('mode=pageEdit', 'mode=preview')}"></iframe>
                </div>
            </div>
        `
    ...
    } else if (view === 'settings.user.update') {
        content = [
            Page({
                title: 'Edit User',
                body: [
    ...
    <div data-body>
        <div data-sidebar data-active="${sidebar}">
            ...
            <div data-nested-sidebar ${view.startsWith('blocks.') ? 'data-active' : ''}>
                ...
                <div data-sidebar-menu>
                    ${sidebarModules(query.id, { permissions })}
                </div>
            </div>
