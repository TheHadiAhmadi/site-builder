        ...
        <div data-name="iframe" data-page-id="${page.id}">
            <div data-content-header>
                ...
                <div data-header-actions>
                    ...
                    ${permissions.page_update || permissions.page_seo ? `<a data-enhance data-button ${page?.dynamic ? 'data-button-icon' : ''} data-button-color="default" href="${getUrl({ view: 'pages.update', id: page?.id, back: encodeURIComponent('?view=iframe' + '&slug='+ encodeURIComponent(slug)) })}">
                    ...
                    ${view === 'iframe' ? `
                        <a data-enhance href="?view=pages.edit&slug=${encodeURIComponent(slug)}" data-button data-button-color="primary">Done</a>
                    ` : `
                        ${permissions.page_content || permissions.page_update ? `<a data-enhance href="?view=iframe&slug=${encodeURIComponent(slug)}" data-button data-button-color="primary">Edit</a>` : ''}
                    `}
                </div>
            </div>

            <div data-iframe-wrapper>
                <iframe class="iframe" src="${slug}?mode=${view === 'iframe' ? 'preview' : 'view'}"></iframe>
            </div>
        </div>
    `
}